# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [1.2.1] - 2026-04-24

### Changed
- Re-implemented Quickselect as an iterative algorithm to prevent stack overflow on massive datasets.
- Optimized `getStructuralMetrics` with an iterative flow and memory-safe cache eviction.

### Added
- Comprehensive stress test suite covering massive payloads, punctuation edge cases, and memory limits.

## [1.2.0] - 2026-04-24

### Added
- Integrated `@veldica/prose-tokenizer` for high-precision, rule-based splitting of sentences and words.
- Implemented size-limited `SYLLABLE_CACHE` (10,000 entries) with FIFO eviction for memory safety.
- Switched to deterministic middle-pivot strategy in Quickselect algorithm.

### Changed
- Improved `getStructuralMetrics` to leverage the full Veldica tokenizer ecosystem.
- Refined `warnings` logic for extreme edge-case inputs.

## [1.1.2] - 2026-04-24

### Added
- `warnings` array in `AnalysisResults` to flag anomalous inputs (e.g., extreme word lengths).
- $O(N)$ Quickselect algorithm for efficient median calculations on massive datasets.

### Changed
- Implemented internal `SYLLABLE_CACHE` to optimize throughput for repetitive common words.
- Upgraded sentence splitter to handle basic abbreviations (U.S.A.) and decimals.
- Capped grade-level scores at 20 to prevent astronomical results from edge-case inputs.

## [1.1.1] - 2026-04-24

### Fixed
- Corrected copyright holder in `LICENSE` from Veldica to Veldica.

## [1.1.0] - 2026-04-24

### Added
- `getStructuralMetrics` bridge utility to generate formula inputs directly from raw text.
- Enhanced `README.md` with "Quick Start" examples for both automatic and manual analysis.
- Explicit documentation for `DALE_CHALL_EASY_WORDS` as a `Set` for performance.

## [1.0.0] - 2026-04-24

### Added
- Initial release of consolidated readability formulas.
- Supported metrics: Flesch Reading Ease, Flesch-Kincaid Grade Level, Gunning Fog, SMOG, Coleman-Liau, ARI, Dale-Chall, Linsear Write, and Type-Token Ratio.
- Deterministic consensus scoring.
- TypeScript support with full type definitions.
- Sub-module exports for optimal tree-shaking.
